...
test.skipIf(!chrome)(
  "visual previews render in StrictMode, zoom independently, and survive navigation",
  async () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        if (path === "/") {
          return new Response(
            '<head><link rel="stylesheet" href="/VisualPreview.browser.css"></head><body><script src="/VisualPreview.browser.js"></script></body>',
    ...
    try {
      const build = await Bun.build({
        entrypoints: [join(import.meta.dir, "VisualPreview.browser.tsx")],
        outdir: dir,
        target: "browser",
      });
